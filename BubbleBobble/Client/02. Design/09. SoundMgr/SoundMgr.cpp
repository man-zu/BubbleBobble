...
{
	m_pSystem = nullptr; 
	ZeroMemory(&m_pChannelArr, sizeof(FMOD_CHANNEL*));
}
...
void CSoundMgr::PlaySound(TCHAR* pSoundKey, CHANNELID eID, float fVolume, bool _check)
{
	...
	if (FMOD_Channel_IsPlaying(m_pChannelArr[eID], &bPlay) && !_check)
	{
		FMOD_System_PlaySound(m_pSystem, FMOD_CHANNEL_FREE, iter->second, FALSE, &m_pChannelArr[eID]);
	}
