...
CStage2::CStage2() : m_player(nullptr)
{
}
...
void CStage2::Initialize()
{
	...
	//m_player->Set_Init_Life(1);
	m_player->Set_Init_Score(CScoreMgr::Get_Instance()->Get_Scene_Score());
	CObjMgr::Get_Instance()->Add_Object(PLAYER, m_player);

	for (int i = 0; i < 5; ++i)
	{
		CObjMgr::Get_Instance()->Add_Object(MONSTER, CAbstractFactory<CBMonsterS3>::Create(300.f + (70.f * i), 400.f));
	}


	// EXTEND
	CObj* pObj = new CBExtend(EXTENDSTATE::X1);
	pObj->Initialize();
	pObj->Set_Pos(200, 550);
	CObjMgr::Get_Instance()->Add_Object(EXTEND, pObj);

	pObj = new CBExtend(EXTENDSTATE::T2);
	pObj->Initialize();
	pObj->Set_Pos(450, 550);
	CObjMgr::Get_Instance()->Add_Object(EXTEND, pObj);
...
void CStage2::Update()
{
	...
	CSceneMgr* tmp = CSceneMgr::Get_Instance();
}
...
void CStage2::Late_Update()
{
	...
	{
		CSceneMgr::Get_Instance()->Scene_Change(CSceneMgr::SC_STAGE3);
		return;
	}
	else
	{
